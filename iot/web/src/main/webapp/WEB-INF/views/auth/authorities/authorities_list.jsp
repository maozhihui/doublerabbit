			...
			<table style="width: 100%;">
				<tr>
					...
					<td style="width: 30%;" valign="top">
						<div class="list_search_head_right" style="position: initial;height: 33px;">
							<security:authorize code="userM_assign">
								<a href="javascript:saveRolePri();" class="btn btn-sm search_a_role" style="background: #e56a49;">
									<i class="search_head_role"></i>
									<span class="search_head_span">保存权限</span>
								</a>
							</security:authorize>				
						</div>
						<div class="panel panel-default multi_headborder panel_min_height" style="margin-top: 0px;">
							<div class="multi_headcolor ">
								<h3 class="panel-title-boostrap">权限列表<span id="roleName_text"></span></h3>
							</div>
							<div id="priList" class="select_tree_div"><div class="op_select_tip">请在左边角色列表中选中角色</div></div>
						</div>
					</td>
					<td style="width: 3px;"> &nbsp;</td>
				</tr>
...
function getRoles(userId,userName,isSupperAdmin){
	...
	$("#roleList").tree({
		...
		onClick : function(node) {//通过角色获取拥有的权限
			var roleId = node.id;
			selectRoleId = roleId;
			var roleName = node.text;
			$("#roleName_text").html('【'+roleName+'】');
			$("#priList").tree({
...
function saveRolePri(){
	if(selectRoleId==-1){
		new alertdialog({title:"提示",content:"请选择需要分配权限的角色！"});
		return;
	}
	...
	$.ajax({
        ...
        datatype: 'json',
        type: "Post",
        data:{roleId:selectRoleId,priIds:priIds},
        success: function (data) {
        	if(data==1){
        		new alertdialog({title:"提示",content:"保存成功！"});
        	}else{
