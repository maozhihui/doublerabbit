					...
					<div class="list_search_head_left">
						...
						<div class="">
							<select class="form-control selectC" name="selectC">
								<option name="name" value="规则名称">规则名称</option>
							</select>
						</div>
						<div class="">
...
function add_rule(){
	rulePageGrid.createModal("新建规则","${ctx}/ruleTenant/to_rule_edit",550,900);
	//window.location.href='${ctx}/ruleTenant/to_rule_edit';
}
function view_rule(){
	...
	rulePageGrid.createModal("查看规则","${ctx}/ruleTenant/rule_view?ruleId="+ids[0],370,900);
	//window.location.href='${ctx}/ruleTenant/rule_view?ruleId='+ids[0];
}
function edit_rule(){
	var ids = rulePageGrid.getSelectedRowIds();
	if(ids.length==0){
	...
	rulePageGrid.createModal("编辑规则","${ctx}/ruleTenant/to_rule_edit?ruleId="+ids[0],550,900);
	//window.location.href='${ctx}/ruleTenant/to_rule_edit?ruleId='+ids[0] ;
}
function del_rule(){
	var ids = rulePageGrid.getSelectedRowIds();
	if(ids.length>0){
...
$(function() {
	...
	rulePageGrid.paging("list","ruleId",columns,'${ctx}/ruleTenant/datasByPage?productId=${CUR_PRODUCT_ID}');
	...
	$("#exportBtn").click(function(){
    	if($(".add-search-div").size()!=0){
    		rulePageGrid.pageExport('${ctx}/ruleTenant/datasByExport',"searchForm",'${ctx}/','规则信息',conditionQuery.getSearchArr());
    	}else{
    		rulePageGrid.pageExport('${ctx}/ruleTenant/datasByExport',"searchForm",'${ctx}/','规则信息'); 
    	}
    });
});
