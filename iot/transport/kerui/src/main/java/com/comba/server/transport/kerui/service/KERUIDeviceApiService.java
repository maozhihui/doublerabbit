package com.comba.server.transport.kerui.service;
...
import com.comba.server.transport.kerui.session.TcpSessionCtx;
import com.comba.server.transport.kerui.session.TcpSessionId;

@Slf4j
@Service
public class KERUIDeviceApiService {
    ...
    //设备注册
    public void subDeviceRegister(IoSession session,String sessionId,String deviceSn,String json){
    	TcpSessionCtx ctx = new TcpSessionCtx(session,processor,authService);
    	BasicRegisterRequest request = JsonConverter.convertToRegisterRequest(deviceSn,new JsonParser().parse(json));
    	DeviceCredentialsFilter filter = new DeviceIdCredentials(deviceSn);
    	
    	try {
			if(ctx.login(deviceSn,filter))
			{
    		...
    		log.error("[{}]invalid token!",deviceSn);
		}catch(DeviceInvalidPasswordException e2){
			log.error("[{}]password error!",deviceSn);
		}catch(DeviceNotExistException e3){
			log.error("[{}]deviceId is not exists!",deviceSn);
